...
import { ArrowLeft, Upload, Trash2, Eye, EyeOff, Grid3X3, List, Search, Download, X, Check } from "lucide-react"

const mockPhotos = [
    {
        id: 1,
        src: "/images/lentes.jpg?height=300&width=300",
        alt: "Foto 1",
...
export default function AlbumManagementPage({ params }: { params: { id: string } }) {
    ...
    const albumTitle = "Momentos Personales" // This would come from the album data
    ...
    const togglePhotoPrivacy = (photoId: number) => {
        setPhotos(photos.map((photo) => (photo.id === photoId ? { ...photo, isPublic: !photo.isPublic } : photo)))
    }

    const deleteSelectedPhotos = () => {
        if (confirm(`¿Estás segura de que quieres eliminar ${selectedPhotos.length} foto(s)?`)) {
    ...
    const deletePhoto = (photoId: number) => {
        if (confirm("¿Estás segura de que quieres eliminar esta foto?")) {
                            ...
                            >
                                <div className="flex items-center space-x-4 flex-1">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-gray-500">Subida el {new Date(photo.uploadDate).toLocaleDateString()}</p>
                                    </div>

                                    <div className="flex items-center space-x-4 text-sm text-gray-500">
                                        <span>{photo.likes} ❤️</span>
                                        <div
                                            className={`flex items-center space-x-1 ${photo.isPublic ? "text-green-600" : "text-red-600"}`}
                                        >
            ...
            {showUploadModal && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl">
                        ...
                        <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-rose-300 transition-colors cursor-pointer">
                            <Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                            <p className="text-gray-600 mb-2">Arrastra tus fotos aquí o</p>
                            <button className="text-rose-500 font-medium hover:text-rose-600">selecciona archivos</button>
                        ...
                        <div className="flex space-x-3 mt-8">
                            ...
                            <button className="flex-1 bg-gradient-to-r from-purple-400 to-lavender-500 text-white py-3 rounded-xl font-medium hover:from-purple-500 hover:to-lavender-600 transition-all">
                                Subir Fotos
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="relative max-w-4xl max-h-full">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 rounded-b-2xl">
                            <div className="flex items-center justify-between text-white">
                                ...
                                <div className="flex items-center space-x-3">
                                    <button
                                        onClick={() => togglePhotoPrivacy(selectedPhoto.id)}
